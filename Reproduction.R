...
lactation <- lactation %>%
mutate(
  lactation_length_days = as.numeric(LacDryOffDate - LacCalvingDate),
  colostrum_length_days = as.numeric(LacColostrumDate - LacCalvingDate)
  )

# Calculate age of cow at first calving
first_calving <- lactation %>%
  filter(LacNumber == 1, !is.na(AniBirthday), !is.na(LacCalvingDate)) %>%
  group_by(AniLifeNumber) %>%
  summarise(age_at_first_calving = interval(AniBirthday, LacCalvingDate) %/% months(1), .groups = "drop")

lactation <- lactation %>%
  left_join(first_calving, by = "AniLifeNumber", relationship = "many-to-many")
